...
export function discover({
  ...
  const pluginPkgDiscovery$ = from(coreContext.env.repoPackages ?? EMPTY).pipe(
    ...
    filter(
      ...
      (pkg) =>
        // ... are NOT under an excluded folder
        config.excludedPluginPaths.every(
          (excludedPath) => !pkg.normalizedRepoRelativeDir.startsWith(excludedPath)
        ) ||
        // ... or that are under an explicitly included folder
        config.includedPluginPaths.some((includedPath) =>
          pkg.normalizedRepoRelativeDir.startsWith(includedPath)
        )
    ),
    map((pkg) => {
